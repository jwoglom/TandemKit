...
class SimulatorAuthProvider: AuthenticationProvider {
    ...
    init(pairingCode: String?, state: PumpStateProvider, authMode: AuthMode) {
        ...
        if authMode == .bypass {
            ...
            if let code = pairingCode, code.count == 6 {
                ...
                jpakeHandler = JpakeServerHandler(pairingCode: code)
    ...
    // MARK: - JPAKE Handlers
    ...
    #if canImport(SwiftECC) && canImport(BigInt)
        private func handleJpake1a(_ request: Jpake1aRequest, context _: HandlerContext) throws -> Message {
        ...
        private func handleJpake1b(_ request: Jpake1bRequest, context _: HandlerContext) throws -> Message {
            guard let handler = jpakeHandler else {
                throw AuthProviderError.jpakeNotInitialized
            }
        ...
        private func handleJpake2(_ request: Jpake2Request, context _: HandlerContext) throws -> Message {
            guard let handler = jpakeHandler else {
                throw AuthProviderError.jpakeNotInitialized
            }
        ...
        private func handleJpake3(_ request: Jpake3SessionKeyRequest, context _: HandlerContext) throws -> Message {
            guard let handler = jpakeHandler else {
                throw AuthProviderError.jpakeNotInitialized
            }
        ...
        private func handleJpake4(_ request: Jpake4KeyConfirmationRequest, context _: HandlerContext) throws -> Message {
            guard let handler = jpakeHandler else {
                throw AuthProviderError.jpakeNotInitialized
            }
    ...
    private func handleCentralChallenge(_ request: CentralChallengeRequest, context _: HandlerContext) throws -> Message {
    ...
    private func handlePumpChallenge(_ request: PumpChallengeRequest, context _: HandlerContext) throws -> Message {
        guard let code = pairingCode, code.count == 16 else {
            throw AuthProviderError.pumpChallengeNotConfigured
        }
...
// MARK: - JPAKE Server Handler
    ...
    class JpakeServerHandler {
        ...
        func processJpake3(_: Jpake3SessionKeyRequest) throws -> Jpake3SessionKeyResponse {
        ...
        func processJpake4(_ request: Jpake4KeyConfirmationRequest) throws -> Jpake4KeyConfirmationResponse {
            ...
            let expectedClientHash = HmacSha256.hmac(
                clientNonce4,
                key: Hkdf.build(nonce: serverNonce3, keyMaterial: derivedSecret)
            )
