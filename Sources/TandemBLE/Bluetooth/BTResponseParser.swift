...
public enum BTResponseParser {
    ...
    @MainActor  static func parse(
        ...
        if let reqMeta = packetArrayList.requestMetadata {
            responseParserLogger
                .debug("[BTResponseParser] request=\(reqMeta.name) needsMore=\(needsMore) opCode=\(packetArrayList.opCode)")
        ...
        if !isValid {
            ...
            responseParserLogger
                .warning(
                    "[BTResponseParser] Validation failed for request=\(reqContext) opCode=\(packetArrayList.opCode) length=\(payload.count)"
                )
        }

        let decodedMessage = decodeMessage(
            opCode: packetArrayList.opCode,
            characteristic: uuid,
        ...
        responseParserLogger
            .error(
                "[BTResponseParser] Unable to decode message for request=\(reqContext) opCode=\(packetArrayList.opCode) payloadLength=\(payload.count)"
            )
    ...
    public static func decodeMessage(opCode: UInt8, characteristic: CBUUID, payload: Data) -> Message? {
        ...
        var candidates = MessageRegistry.bestMatches(
            opCode: opCode,
        ...
        if candidates.isEmpty {
            responseParserLogger
                .warning(
                    "[BTResponseParser] No registry candidate for opCode=\(opCode) characteristic=\(characteristic.uuidString) payloadLength=\(payload.count)"
                )
        }
        guard let meta = candidates.first else { return nil }
        if payload.count != Int(meta.size) {
            responseParserLogger
                .debug("[BTResponseParser] Candidate \(meta.name) expects size=\(meta.size) but payload=\(payload.count)")
        }
    ...
    private static func checkCharacteristicUuid(_ uuid: CBUUID, output: Data) {
        let allowed = [
            CharacteristicUUID.AUTHORIZATION_CHARACTERISTICS.cbUUID,
