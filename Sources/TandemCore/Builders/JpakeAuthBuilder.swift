import Foundation
    ...
    public class JpakeAuthBuilder {
        private static var instance: JpakeAuthBuilder?
        ...
        init(
            ...
            cli = EcJpake(role: .client, password: JpakeAuthBuilder.pairingCodeToBytes(pairingCode), random: rand)
            sentMessages = []
            receivedMessages = []
            self.step = step
        ...
        convenience init(pairingCode: String) {
            self.init(
                ...
                step: JpakeAuthBuilder.decideInitialStep(derivedSecret: nil),
                clientRound1: nil,
                serverRound1: nil,
                clientRound2: nil,
                serverRound2: nil,
                derivedSecret: nil,
                rand: JpakeAuthBuilder.defaultRandom
            )
        }
        ...
        convenience init(pairingCode: String, derivedSecret: Data) {
            self.init(
                pairingCode: pairingCode,
                step: JpakeAuthBuilder.decideInitialStep(derivedSecret: derivedSecret),
                clientRound1: nil,
                serverRound1: nil,
                clientRound2: nil,
                serverRound2: nil,
        ...
        public func nextRequest() -> Message? {
            ...
            case .CONFIRM_3_RECEIVED:
                ...
                let hashDigest3 = HmacSha256.hmac(
                    clientNonce4!,
                    key: Hkdf.build(nonce: serverNonce3!, keyMaterial: derivedSecret!)
                )
                request = Jpake4KeyConfirmationRequest(
                    appInstanceId: 0,
                    nonce: clientNonce4!,
                    reserved: Jpake4KeyConfirmationRequest.RESERVED,
                    hashDigest: hashDigest3
                )
                step = .CONFIRM_4_SENT
                print("[JpakeAuthBuilder] produced Jpake4KeyConfirmationRequest")
            case .CONFIRM_4_RECEIVED:
                let hashDigest4 = HmacSha256.hmac(
                    serverNonce4!,
                    key: Hkdf.build(nonce: serverNonce3!, keyMaterial: derivedSecret!)
                )
        ...
        static func defaultRandom(_ count: Int) -> Data {
            NonBlockingRandom.shared.next(count: count)
        }
    }
...
private final class NonBlockingRandom {
    ...
    func next(count: Int) -> Data {
        ...
        var data = Data(count: count)
            ...
            print(
                "[NonBlockingRandom] Warning: SecRandomCopyBytes failed with \(result), falling back to SystemRandomNumberGenerator"
            )
        #endif
