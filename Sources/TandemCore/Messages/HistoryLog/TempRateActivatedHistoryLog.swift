...
public class TempRateActivatedHistoryLog: HistoryLog {
    ...
    public required init(cargo: Data) {
        percent = Bytes.readFloat(cargo, 10)
        duration = Bytes.readFloat(cargo, 14)
        tempRateId = Bytes.readShort(cargo, 20)
        super.init(cargo: cargo)
    }

    public init(pumpTimeSec: UInt32, sequenceNum: UInt32, percent: Float, duration: Float, tempRateId: Int) {
        let payload = TempRateActivatedHistoryLog.buildCargo(
